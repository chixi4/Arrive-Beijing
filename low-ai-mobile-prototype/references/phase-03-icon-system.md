...
- Pixel diffs, CSV masks, and SVG replica checks prove technical fit only. They do not prove the icon is aesthetically or semantically good.

## Generation Review Policy

- Treat the image model as stateless. Every prompt must be self-contained; never rely on phrases like "same as before", "like version 05", or "the previous original".
- For thicker icons, prefer positive style wording such as bold black outline, rounded stroke, simple geometric form, sparse details, and low visual complexity. Do not rely on post-processing stroke expansion as the main way to make icons bolder.
- When returning to a simpler/original feel, remove extra style triggers first. Avoid piling up negative constraints unless the model is repeatedly adding a specific unwanted feature.
- During prompt exploration, generate candidates only. Do not clean, slice, convert, integrate, or commit assets until the user selects a target or explicitly asks the agent to self-approve.
- Before integration, make an explicit aesthetic pass: simplicity, line weight, original-family resemblance, semantic clarity, optical balance, and absence of extra detail.
- Station icons should not be regenerated by default. Reuse project station icon assets unless the user explicitly asks to regenerate them.

## Work Modes

1. **Prompt exploration**: create candidate boards or single icons, compare visually, and explain prompt differences. Do not modify runtime assets.
2. **Approved target**: after user approval, clean, slice, simulate masks, and convert to currentColor SVG replicas.
3. **Runtime integration**: after technical checks pass, wire approved icons into the registry and pages. Record both the technical checks and the approval basis.
...
The prompt must include the intended style, line weight, simplicity level, icon list, and grid/canvas requirements even if those details were discussed earlier in the conversation.

If the first result is thin, inconsistent, decorative, too complex, too different from the target family, or includes text, regenerate with adjusted positive wording. Do not continue with a weak target.
...
- Passing this rule only means the SVG replica matches the cleaned target. It does not override user review or aesthetic judgment.
...
- Generated icons were either approved by the user or explicitly self-approved under a user request to do so.

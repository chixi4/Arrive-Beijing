...
- 本轮把 `#/station/select` 与 `#/station/switch` 都重做成统一的横向滑动站点卡片：保留 10 个站点中文名，卡片内嵌站点建筑位图和原始站点图标，底部只保留已选站点摘要与确认按钮，不再出现原稿多余站名胶囊。
...
- 首页头图不再使用通用开屏图，而是跟随当前站点使用同一张站点卡片建筑图；大兴机场站点卡片左下角图标明确读取 `P04_icon_大兴机场.png` 并加版本号避开缓存。

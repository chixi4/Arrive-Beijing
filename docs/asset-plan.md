...
站点选择页、站区切换页、司机排队列表和相关站点清单必须沿用这套顺序，不能为了让当前站点显示在顶部而重排数组；进入页面时只通过滚动位置聚焦当前站点。

运行版站点照片规则：

- 文件统一放在 `assets/bitmap/stations-v3/`，页面通过 `stationHeroAssets` / `stationPortraitAssets` 引用。
- 当前槽位共用同一张 WebP，通过 `object-fit: cover` 适配首页头图和选择卡片。
- 新增真实照片优先压到约 `960px` 宽；不把原始 zip 或多余候选照片提交进运行分支。
- 若临时复用旧图标 mask，必须在 `docs/ui/icon-style.md` 写明别名关系，后续再单独重做 3x3 站点图标板。

...
本项目图标统一走 `ICON_LIBRARY -> iconMarkup(name) / anchorIcon(name)`，最终在页面里渲染为 `currentColor` 线性 SVG。生图模型只作为风格校准和像素比对目标，不作为中文文案来源，也不从页面截图里扣图标。

## 当前结论

- 当前库：52 个 SVG 图标，5 个语义别名。
- 当前使用：49 个语义图标，0 个缺失。
- 已替换的明显误用：`我的积分` 不再用扫码图标，`监督投诉邮箱` 不再用扫码图标，`场站接驳` 不再复用公交图标，`自驾停车` 不再复用小汽车图标，`退出登录` 不再用扫码图标。
- 仍需重点校准的高频图标：`pin`、`user`、`map`、`home`、`search`、`notice`、`taxi`、`car`、`route`。
